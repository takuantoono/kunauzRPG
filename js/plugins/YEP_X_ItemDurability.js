...
DataManager.processIDurNotetags3 = function(group) {
  ...
  for (var n = 1; n < group.length; n++) {
    ...
    if ([1, 2, 3, 4, 5, 6].contains(obj.scope)&&!obj.meta.notdur) {
...
ItemManager.makeDurability = function(item, variance) {
    ...
    if (variance >= 0&&!item.meta.no_count) this.makeDurabilityVariance(item, variance);
...
Game_Actor.prototype.durabilityBreakItem = function(obj) {
    ...
    $gameParty.gainItem(obj, -1, false)
    return
    }
...
Game_Actor.prototype.damageRandomDurability = function(value, group) {
    ...
    if($gameActors.actor(1).isStateAffected(344))item.durability += value;
    if (item.durability <= 0){
    if (item.meta.norepair){
    this.damageShieldDurability(item)
    }else{
...
Game_Actor.prototype.damageWeaponDurability = function(item, num) {
    ...
    this.damageShieldDurability(item)
     ...
     this.durabilityBreakItem3(item, num);
     }
     }
};
