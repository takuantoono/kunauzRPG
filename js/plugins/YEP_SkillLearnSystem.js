...
Game_Actor.prototype.sufficientJpLearnSkill = function(skill, classId) {
  ...
  if($gameSwitches.value(434))jpCost = Math.floor(jpCost/2);
...
Game_Actor.prototype.canLearnSkill = function(skill, classId) {
    ...
    var gcost = skill.learnCostGold;
    var acost = 0;
    if($gameSwitches.value(434)){
    acost = gcost;
    gcost *= 5;
    }
    if (gcost > $gameParty.gold()) return false;
    if (acost > $gameVariables.value(98)) return false;
    if ($gameSwitches.value(502)) return false;
...
Game_Actor.prototype.customLearnSkillJpCost = function(skill) {
    ...
    var mcost = -1 * item.learnCostJp*2/3;
    var a = this;
    var b = this;
    var user = this;
    var target = this;
...
Window_SkillLearn.prototype.meetsRequirements = function(skill) {
    ...
    if(this._actor._actorId==5) return false;
...
if ($gameSwitches.value(779)) return false;
    ...
    var evalValue = this.getEvalLine(item.learnRequireEval,item);
    if (evalValue !== undefined) return evalValue;
    return true;
};

Window_SkillLearn.prototype.getEvalLine = function(evalLine,item) {
...
Window_SkillLearnData.prototype.drawGoldCosts = function(wy) {
    ...
    var gcost = this._skill.learnCostGold;
    if($gameSwitches.value(434))gcost *= 5;
    if(this._skill.meta.mage&&$gameSwitches.value(502))gcost *= 5;
    var costText = Yanfly.Util.toGroup(gcost);
    this.contents.fontSize = Yanfly.Param.SLSCostSize;
    if (gcost > $gameParty.gold()) {
...
Window_SkillLearnData.prototype.drawApCosts = function(wy) {
    if(this._skill.meta.mage&&$gameSwitches.value(502)){
    }else{
    if(!$gameSwitches.value(434))return wy;
    }
    var text = 'AP ';
    var wx = this.drawTextEx(text, 0, wy);
    var ww = this.contents.width - wx - 4;
...
Window_SkillLearnData.prototype.drawJpCosts = function(wy) {
    ...
    var cost = this._skill.learnCostJp;
    cost += this._actor.customLearnSkillJpCost(this._skill);
    if($gameSwitches.value(434))cost = Math.floor(cost/2);
    if(this._skill.meta.mage&&$gameSwitches.value(502))cost = Math.floor(cost/2);
    if (cost <= 0) return wy;
    var text = '';
...
Scene_Skill.prototype.processLearnSkill = function(skill, classId) {
  ...
  var gcost = this._skill.learnCostGold;
  if($gameSwitches.value(434)){
  $gameVariables.setValue(98, $gameVariables.value(98) - gocost)
  gcost *= 5;
  }
  
  $gameParty.loseGold(gcost);
...
Scene_LearnSkill.prototype.processLearnSkill = function(skill, classId) {
  ...
  var gcost = skill.learnCostGold;
  if($gameSwitches.value(434)){
  $gameVariables.setValue(98, $gameVariables.value(98) - gcost)
  gcost *= 5;
  }
  $gameParty.loseGold(gcost);
  $gameParty.processLearnSkillCost(skill);
