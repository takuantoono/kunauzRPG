...
Window_AugmentItemList.prototype.containsType = function(item) {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      if (text.match(/ITEM[ ](\d+)/i)) {
        ...
        if(augment.meta.shinmyaku == 1 && item.meta.shinmyaku == 1 && item.id != augment.id)return false
      }
    }
    if(item.meta.ashield){
    if(!DataManager.isWeapon(this._item)||!item.meta.shinmyaku){
    if(this._item.atypeId == 5 || this._item.atypeId == 6){
    return true
    }else{
    return false
    }
    ...
    if(!item.meta.shinmyaku && !this._item.meta.metal && item.meta.marmor)return false
    if(!item.meta.shinmyaku && DataManager.isArmor(this._item) && item.meta.aarmor){
    if(this._item.atypeId == 5 || this._item.atypeId == 6){
    return false
    }else{
    return true
