...
Game_Party.prototype.populateCraftingRecipes = function() {
	...
	for(var i = 1; i < lengthOfItemList; i++){
		...
		if(typeof $dataItems[i].meta.comboIngredientH !== "undefined"){
		if($gameSwitches.value(440)){
		
			var ingredientArrayIngredient = $dataItems[i].meta.comboIngredientH.split(',');
			console.log(ingredientArrayIngredient)
			for(var j=0; j<2; j++) {
				ingredientArrayIngredient[j] = parseInt(ingredientArrayIngredient[j], 10);};
				ingredientArray.push(ingredientArrayIngredient);
		}
		...
		if(typeof $dataItems[i].meta.comboChance !== "undefined"){
			Game_Party.prototype._craftingRecipes[$dataItems[i].id] = ingredientArray;
		};
	};		
};
...
Game_Party.prototype.canCombine = function(recipeKeyName){
	...
	var aps = false
	if(recipeDetails[1]<=$gameVariables.value(98))aps = true
	if($gameSwitches.value(440))aps = true
	return (haveAllIngredients === $gameParty.ingredients(recipeKeyName).length)&&aps;
};
...
craftingListWindow.prototype.addOriginalCommands = function() {
	...
	for(var i = 0; i < numberOfLearnedRecipes.length; i++){
		...
		this.addCommand($dataItems[itemNumber].name, recipeKeyName, enabled);
	};
 };
...
craftingDetailsWindow.prototype.update = function() {
	...
	if(!$gameSwitches.value(440))this.drawComboChance(0, this.windowHeight() - this.lineHeight() * 5,this.windowWidth());
};
...
craftingDetailsWindow.prototype.drawDetailsName = function(x, y, width) {
	...
	this.resetTextColor();
	this.makeFontBigger();
	this.drawText($dataItems[itemNumber].name, x+100, y, width - 32, 'left');
	this.makeFontSmaller();
	
	///Draw description
	this.drawSmallTextEx($dataItems[itemNumber].description, x, y + 100 + 12, width - 32);
	//this.drawText($dataItems[itemNumber].description, x, y + 100 + 12, width - 32, 'center');
    this.changeTextColor(this.systemColor());
	

	this.drawIcon($dataItems[itemNumber].iconIndex, 200, 60);
};

Window_Base.prototype.drawSmallTextEx = function(text, x, y) {
    if (text) {
...
craftingDetailsWindow.prototype.drawIngredient = function(x, y, width) {
    ...
    this.drawText(this.japList(2001) + _myItemCnt, 350, 80, 300, 60);
	for(i = 0;i < ingredientArray.length; i++){
		y = Y + 32*i + 6;
		var itemNumber = ingredientArray[i][0];
		var requiredAmount = ingredientArray[i][1];
...
Scene_CraftingMenu.prototype.create = function() {
   ...
   $gameParty.populateCraftingRecipes();
...
Scene_CraftingMenu.prototype.onDetailsOk = function() {
	...
	if($gameParty.canCombine(recipeKeyName)){
		...
		AudioManager.playSe({"name":"Absorb2","volume":90,"pitch":90,"pan":0})
		if(!$gameSwitches.value(440)){
		var aa =$gameVariables.value(98)-recipeDetails[1]
		$gameVariables.setValue(98,aa)
		}
		if($dataItems[itemNumber].meta.rulet){
		var wakoku =$gameVariables.value(402)+Math.floor($dataItems[itemNumber].meta.rulet)
		$gameVariables.setValue(402,wakoku)
		}
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === "itemComboLearn") {
		//var arg = $dataItems[JSON.parse(args[0])].name+$dataItems[JSON.parse(args[0])].id;
		var arg = $dataItems[JSON.parse(args[0])].id;
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === "itemComboChanceChange") {
		var arg1 = $dataItems[JSON.parse(args[0])].id;
